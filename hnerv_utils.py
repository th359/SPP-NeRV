...
def combine_images(images, block_size=2):
    N = block_size
    assert len(images) == N*N
    C, H, W = images[0].shape
    out = torch.zeros(C, H*N, W*N, dtype=images[0].dtype, device=images[0].device)
    idx = 0
    for i in range(N):
        for j in range(N):
            out[:, i::N, j::N] = images[idx]
            idx += 1
    return out
...
class VideoDataSet(Dataset):
    ...
    def split_image(self, img):
        C, H, W = img.shape
        N = self.split_num
        assert H%N == 0 and W%N == 0, 'H and W must be multiples of split_num'
        patches = img.unfold(1, N, N).unfold(2, N, N)
        parts = []
        for i in range(N):
            for j in range(N):
                parts.append(patches[..., i, j])
        return parts

    def __len__(self):
        return len(self.samples) * self.split_num * self.split_num

    def __getitem__(self, idx):
        img_idx = idx // (self.split_num*self.split_num)
        norm_img_idx = float(img_idx+1) / len(self.samples)
        part_idx = idx % (self.split_num*self.split_num)
        norm_part_idx = float(part_idx+1) / (self.split_num*self.split_num)
        ...
        if self.embed_inter:
            ...
            return {'img': img, 'img_idx': img_idx, 'norm_img_idx': norm_img_idx, 
                    'part': part_img, 'part_idx':part_idx, 'norm_part_idx': norm_part_idx}
...
def loss_fn(pred, target, loss_type='L2', batch_average=True, img_data=None, split_num=None):
    ...
    if loss_type == 'L2':
        ...
        N = split_num
        patches = img_data.detach().unfold(2, N, N).unfold(3, N, N)
        weight = torch.full_like(target, 1e-6)
        weight = weight + 1e-6
        for i in range(N):
            for j in range(N):
                weight = weight + torch.abs(target - patches[..., i, j])
...
def debug(args):
    ...
    for i in range(132*4):
        ...
        part_img = sample['part']
        part_idx = sample['part_idx']
        norm_part_idx = sample['norm_part_idx']
        print(f'i: {i}')
        print(f'img : {img.shape}, idx: {img_idx}, norm: {norm_img_idx}')
        print(f'part: {part_img.shape}, idx: {part_idx}, norm: {norm_part_idx}')
